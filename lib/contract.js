...
class Contract {
  ...
  static get web3RinkebyProvider() {
    return new HDWalletProvider(config.network.account.key, config.network.http, 0)
    // return new Web3.providers.HttpProvider( config.network.http )
  ...
  static get web3Provider() {
    return env === 'pro' ? this.web3RinkebyProvider : this.web3HttpProvider;
  }
  ...
  static get contractDefaults() {
    return {
      from: this.mainAccount.address, //this.accounts[1] || this.mainAccount.address, //config.accounts[0].address, //
      gas: 6500000,
      gasPrice: 10000000000//100000000000,
    }
  }
